...
def generate_occlusion(image_shape, occlusion_shape, x, y):
  occ_image = np.zeros(image_shape)
  occlusion = np.array([[0.5 for _ in range(image_shape[1])] for _ in range(image_shape[0])])
  occ_image[y:y+occlusion_shape[1],x:x+occlusion_shape[0]] = occlusion
  occ_image = np.zeros(IMAGE_SHAPE) + occ_image[:,:,None]
  print(occ_image)
  return occ_image

def generate_occlusions(image_shape, occlusion_shape, stride):
  occlusions = []
  for y in range(image_shape[0])[::stride]:
    for x in range(image_shape[0])[::stride]:
      occlusions.append(generate_occlusion(image_shape, occlusion_shape, x, y))
...
if __name__ == '__main__':
    ...
    occlusion = generate_occlusion(IMAGE_SHAPE[:-1], (10,10), 0, 0)
    input()
    ...
    for i, (state, floor_data) in enumerate(frame_data):
        ...
        # rgb_image = superimpose(image.astype(np.float32), rgb_saliency_map.astype(np.float32))
